...
class CarLogic {
  ...
  static double getFinalDiscountSD(
    ...
    if (isApi) {
      finalDiscount = getFinalDiscountSd(vendor!, price!);
    } else {
      if (weekdayperhr == 0) {
        finalDiscount =
  ...
  static double getFinalDiscountSd(Vendor vendor, int price) {
    final double finalPrice = price * (1 + vendor.taxRate!) * (vendor.currentRate as double);
    return finalPrice;
  }
  ...
  static double getFinalDiscountOs(Vendor vendor,
      {required int hours, required int ratePerKm, required double distance, int? driverCharges}) {
  ...
  static double getFinalDiscountAt(Vendor vendor, int price) {
    final double finalDiscount =
        price * vendor.currentRate! * (1 + (vendor.taxRate as double));
    return finalDiscount;
  }
