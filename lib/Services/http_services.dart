...
class HttpServices {
  ...
  static Future<http.Response> getDistance(
    ...
    return http.get(Uri.parse(distanceUrl)).timeout(
          timeOutDuration,
        );
  }
  ...
  try {
    ...
    print(data);
    if (!data['status']) {
      print('MyChoize API Request Failed: false returned');
    }
  } catch (e) {
    print('MyChoize API Cancellation Request Failed: ${e.toString()}');
    return false;
  }
  return true;
}
