...
class WowCarServices {
  ...
  static Future<List<CarModel>> getMonthlyRentalCars({Vendor? vendor}) async {
    final List<CarModel> list = [];
    try {
      final http.Response response =
          await http.get(Uri.parse(wowCarsMonthlyUrl)).timeout(
                timeOutDuration,
              );
      ...
      data.forEach((element) {
        ...
        list.add(CarModel.fromJson(element, vendor, DriveTypes.SUB));
      });
      return list;
    } catch (e) {
      return list;
  ...
  static Future<List<CarModel>> getWowCarSD(DriveModel model,
      {Vendor? vendor}) async {
    final List<CarModel> list = [];
    try {
      ...
      final http.Response response =
          await http.get(Uri.parse(wowCarsSelfDriveUrl + parameters)).timeout(
                timeOutDuration,
              );
