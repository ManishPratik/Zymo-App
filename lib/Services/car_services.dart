...
class CarServices {
  ...
  Future<List<CarModel>> getCars(DriveModel model) async {
    try {
      ...
      print('CAR SERVICES ERROR: ${e.toString()}');
  ...
  Future<List<CarModel>> getSelfDriveCars(DriveModel model, Map<String, Vendor> vendors, List<CarModel> carsData) async {
    final List<Future<List<CarModel>>> apiCalls =
  ...
  Future<Map<String, Vendor>> getVendor(DriveTypes type) async {
    ...
    vendorData.docs.forEach((element) {
      ...
      final Vendor vendor = Vendor.fromJson(data, type);
  ...
  List<CarModel> parseCarData(QuerySnapshot<Map<String, dynamic>> cars,
    ...
    return cars.docs.map((doc) {
      try {
        final data = doc.data();
        final vendorName = data['vendor']?.toString().toLowerCase();
        final vendor = vendorName != null ? vendorList[vendorName] : null;

        if (vendor == null) {
          debugPrint('Vendor not found: ${data['vendor']}');
          return null;
        }

        return CarModel.fromJson(data, vendor, model.drive!, driveModel: model);
      } catch (e) {
        debugPrint('Error parsing car data: $e');
        return null;
      }
  ...
  static String getDrive(DriveModel model) {
    return model.drive == DriveTypes.AT ? model.type!
        : Vendor(
      advancePay: 0,
      name: '',
      imageUrl: '',
