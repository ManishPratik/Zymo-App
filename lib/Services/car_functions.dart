...
class CarFunctions {
  ...
  late final DateTime oneYearFromNow = now.add(const Duration(days: 365));

  /*OMASAY: I have found this to not be suitable for working , therefore switching to checker variable.
  ...
  void startTimePicker(BuildContext context) {
    final prov = Provider.of<CarProvider>(context, listen: false);
  ...
  Future<void> startDatePicker(BuildContext context) async {
    ...
    if (picked != null) {
      //selectedDate = picked; // Update _selectedDate
      //onSelectedStartDate(picked);

      prov.setStartDate(picked); // Pass both arguments
    }
  }

  Future<void> endDatePicker(BuildContext context) async {
    final prov = Provider.of<CarProvider>(context, listen: false);
    final start = prov.startDate;
    final end = prov.endDate;

    print(start);
    if (start != end && checker == 0) {
      checker = 1;
    ...
    if (picked != null) {
      ...
      prov.setEndDate(picked); // Pass both arguments
    }
  }

  bool validateMonthlyRental(BuildContext context, CarProvider provider, {required int minimumBookingDuration}) {
    bool isValid = false;
    const hours2 = 8;
  ...
  static Future<String> getCity(String type, String location) async {
    ...
    if (location.contains('Maharashtra')) {
      ...
      city = 'Mysore';
    } else if (location.contains('Telangana')) {
      city = 'Hyderabad';
  ...
  Future<void> selfDriveNavigate(BuildContext context) async {
  ...
  Future<double> getDistance(
    ...
    final distanceInKm = double.tryParse(_distance.split(" ").first) ?? 0;

    final double minDistance = type == DriveTypes.RT ? 200 : 50; // Example values

    return distanceInKm < minDistance ? minDistance : distanceInKm;
  }
